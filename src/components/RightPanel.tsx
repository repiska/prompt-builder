  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div>
          <div className="label">Live prompt</div>
          <div className="text-sm text-ink-200 truncate">
            {recipe.name}
            {recipe.custom && <span className="ml-2 text-xs text-warn">modified</span>}
          </div>
        </div>
      ...
      {hasErrors && (
        <div className="mb-3 p-2.5 rounded-md bg-bad/10 border border-bad/40 text-bad text-xs">
          Resolve errors in the validation panel before sending this prompt.
        </div>
      )}
...
{text || (pass === 'pass2' ? '(no Pass 2 — add a GRADE block)' : '')}
        </pre>
        <div className="border-t border-ink-700 p-2.5 flex items-center justify-between gap-2">
          ...
          <div className="flex gap-1">
            ...
            >
              {copied === 'current' ? 'Copied ✓' : `Copy ${pass === 'pass1' ? 'Pass 1' : 'Pass 2'}`}
            </button>
            {composed.pass2 && (
              <button
                type="button"
      ...
      <details className="mt-3">
        <summary className="text-xs text-ink-300 cursor-pointer hover:text-ink-100">
          Per-block breakdown ({composed.perBlock.length})
        </summary>
